...
**Both were fixed when D-0035 was taken (row D9): `newLine: "lf"` and a `.gitattributes` holding
`src/**` and `dist/**` to LF.** The two paragraphs above are kept as the reason those two lines exist.
...
| D9 | Does cadenza harden its own output so a rebuilt pack is platform-independent — `newLine: "lf"` in `tsconfig.build.json`, and a `.gitattributes` holding `src/` (and `dist/`) to LF? | **TAKEN, and implemented with the entry.** `newLine: "lf"` in `tsconfig.build.json` and a `.gitattributes` holding `src/**` and `dist/**` to LF. Still not a precondition of the bridge: form 2a does not need it | It is small, it is the difference between form 2b working everywhere and working on one platform, and it is wanted anyway by any future route (b). It is deliberately not folded into the bridge decision: 2a does not need it, so making it a precondition would price the recommendation above what it costs. A gate that wants 2b as the default should take D9 with it |
